...
import { generateProblemAndTasks, generatePitchDeck } from '@/lib/ai-service';
...
export default function ProjectDetailScreen() {
  ...
  // Pitch Deck Modal
  const [pitchDeckVisible, setPitchDeckVisible] = useState(false);
  ...
  const handleGeneratePitchDeck = async () => {
    ...
    try {
      ...
      const result = await generatePitchDeck(project.name, project.problemStatement || 'A hackathon project.', features, tasks);
      setPitchDeckContent(result.pitchDeck);
      let rawMermaid = result.mermaidDiagram || '';
      rawMermaid = rawMermaid.replace(/```mermaid/gi, '').replace(/```/g, '').trim();
      setDiagramMermaid(rawMermaid);
      setPitchDeckVisible(true);
    } catch (e: any) {
      Alert.alert('Error', e.message);
    } finally {
  ...
  const handleExportPDF = async () => {
    try {
      const htmlSlides = pitchDeckContent
        .split('## Slide')
        .map((slide, index) => {
          if (index === 0) {
            return `<div class="slide title-slide">
              <h1>${slide.replace('# Pitch Deck:', '').replace('#', '').trim()}</h1>
              <p class="subtitle">Generated by HackMate AI</p>
            </div>`;
          }
          
          const lines = slide.split('\n').filter(l => l.trim() !== '');
          const slideTitle = lines[0].replace(':', '').trim();
          let bodyHtml = lines.slice(1).map(l => {
            if (l.startsWith('- ')) return `<li>${l.substring(2)}</li>`;
            return `<p>${l}</p>`;
          }).join('');
          
          bodyHtml = bodyHtml.replace(/(<li>.*<\/li>)/g, '<ul>$1</ul>');
      ...
      const architectureSlide = diagramMermaid ? `
        <div class="slide title-slide" style="background-color: #ffffff;">
          <h2>Architecture Diagram</h2>
          <div class="content" style="text-align: center; margin-top: 40px;">
            <img src="https://quickchart.io/mermaid?graph=${encodeURIComponent(diagramMermaid)}" style="max-width: 100%; height: auto; max-height: 60vh;" />
          </div>
        </div>
      ` : '';
      ...
      const html = `
        <html>
          <head>
            ...
            <style>
              body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; margin: 0; padding: 0; color: #1f2937; background-color: #f9fafb; }
              .slide { page-break-after: always; min-height: 90vh; padding: 60px; box-sizing: border-box; background: white; margin: 20px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); }
              .title-slide { display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; }
              h1 { color: #7C3AED; font-size: 48px; margin-bottom: 20px; font-weight: 800; }
              .subtitle { font-size: 24px; color: #6b7280; }
              h2 { color: #7C3AED; font-size: 32px; border-bottom: 3px solid #f3f4f6; padding-bottom: 16px; margin-bottom: 30px; margin-top: 0; }
              p { font-size: 20px; line-height: 1.6; margin-bottom: 16px; color: #4b5563; }
              ul { margin-bottom: 20px; padding-left: 30px; }
              li { font-size: 20px; line-height: 1.6; margin-bottom: 12px; color: #4b5563; }
  ...
  return (
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {activeTab === 'overview' && (
          <View style={styles.tabContent}>
            ...
            <TouchableOpacity style={[styles.actionCard, { marginTop: -4 }]} onPress={handleGeneratePitchDeck} disabled={generatingPitch}>
              <View style={[styles.actionIconBox, { backgroundColor: C.amber + '20' }]}>
                <Layout size={20} color={C.amber} />
              </View>
              <View style={styles.actionCardText}>
                <Text style={styles.actionCardTitle}>Generate Pitch Deck</Text>
                <Text style={styles.actionCardSub}>Create a 5-slide presentation outline.</Text>
              </View>
              {generatingPitch ? <ActivityIndicator color={C.amber} /> : <ChevronLeft size={16} color={C.textMuted} style={{ transform: [{ rotate: '180deg' }] }} />}
            </TouchableOpacity>
          </View>
        )}
      ...
      {/* ── Pitch Deck Modal ── */}
      <Modal visible={pitchDeckVisible} transparent animationType="slide" onRequestClose={() => setPitchDeckVisible(false)}>
        <View style={styles.modalOverlay}>
          <TouchableOpacity style={StyleSheet.absoluteFill} onPress={() => setPitchDeckVisible(false)} />
          <View style={[styles.modalSheet, { paddingBottom: Math.max(insets.bottom, 20), height: '85%' }]}>
            <View style={styles.modalHandle} />
            <Text style={styles.modalTitle}>Pitch Deck & Architecture</Text>
            
            <Text style={[styles.cardDesc, { marginBottom: 16 }]}>Long-press the text below to select and copy it to your presentation software!</Text>

            <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
              <View style={[styles.input, { padding: 16, minHeight: 200, paddingBottom: 24 }]}>
                <Text 
                  style={{ fontFamily: 'monospace', fontSize: 13, lineHeight: 22, color: C.text }} 
                  selectable={true}
                >
                  {pitchDeckContent || 'Generating...'}
                </Text>
              </View>
              
              {!!diagramMermaid && (
                <View style={{ marginTop: 8, marginBottom: 16 }}>
                  <Text style={[styles.label, { marginBottom: 12 }]}>Architecture Diagram</Text>
                  <Image 
                    source={{ uri: `https://quickchart.io/mermaid?graph=${encodeURIComponent(diagramMermaid)}` }} 
                    style={{ width: '100%', height: 250, resizeMode: 'contain', backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: C.border }} 
                  />
                </View>
              )}
            </ScrollView>
            ...
            <View style={{ flexDirection: 'row', gap: 12, marginTop: 16 }}>
              <TouchableOpacity style={[styles.primaryBtn, { flex: 1, backgroundColor: C.surfaceHigh, borderWidth: 1, borderColor: C.border }]} onPress={() => setPitchDeckVisible(false)}>
                <Text style={[styles.primaryBtnText, { color: C.text }]}>Close</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.primaryBtn, { flex: 1, backgroundColor: C.amber }]} onPress={handleExportPDF}>
                <Text style={[styles.primaryBtnText, { color: '#000' }]}>Export PDF</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
